...
const DoctorCard = ({ doctor }) => {
  return (
    <div className="p-5 bg-white shadow-md hover:shadow-xl hover:scale-105 transition-all duration-500 ease-in-out rounded-xl border border-gray-200">
      {/* Doctor Image */}
      <img src={doctor.photo} className="w-full rounded-t-lg" alt={doctor.name} />

      {/* Doctor Name */}
      <h2 className="text-xl font-bold mt-4 text-gray-800">{doctor.name}</h2>
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-700">
            {doctor.avgRating} <span className="text-gray-500 text-sm">({doctor.totalRating})</span>
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">At {doctor.hospital}</p>
        </div>
